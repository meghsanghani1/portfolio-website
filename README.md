...
The menu uses eight staggered vertical wipe blocks, a reversible GSAP timeline loaded from the pinned npm distribution, and a CSS fallback if the library cannot load. It respects reduced motion. The menu labels use your site content: About, Experience, Expertise, Case Studies, and Contact. The fluid cursor is retained from the earlier website and bundled into `index.html`.
