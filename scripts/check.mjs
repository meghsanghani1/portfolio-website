...
if ((page.match(/class="megh-menu-name"/g) ?? []).length !== 6) errors.push('Navigation links are missing.');
for (const anchor of ['about','experience','platforms','ai-tools','case-studies','contact']) {
  if (!new RegExp(`<a class="megh-menu-item"[^>]*href="#${anchor}"`).test(page)) errors.push(`Menu target missing: ${anchor}`);
}
if (page.includes('[ EXPLORE / MEGH SANGHANI ]') || page.includes('Strategy / Performance / Growth')) errors.push('Old menu captions remain.');
